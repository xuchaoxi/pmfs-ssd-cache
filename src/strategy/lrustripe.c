...
static volatile void *addToLRUStripeHead(NVMStripeBufferDescForLRU *nvm_buf_hdr_lru)
{
    ...
    {
        nvm_stripe_control_lru->first_lru = nvm_buf_hdr_lru->lru_buf_id;
        nvm_stripe_control_lru->last_lru = nvm_buf_hdr_lru->lru_buf_id;
    }
    else {
        nvm_buf_hdr_lru->next_lru = nvm_stripe_descriptors_lru[nvm_stripe_control_lru->first_lru].lru_buf_id;
        nvm_buf_hdr_lru->last_lru = -1;
        nvm_stripe_descriptors_lru[nvm_stripe_control_lru->first_lru].last_lru = nvm_buf_hdr_lru->lru_buf_id;
        nvm_stripe_control_lru->first_lru = nvm_buf_hdr_lru->lru_buf_id;
    }
    return NULL;   
}
...
NVMBufferDescStripe *getLRUStripeBuffer(NVMBufferTag nvm_buf_tag)
{
    ...
    if(nvm_buffer_control->first_freenvm >= 0)
    {
        ...
        long lru_buf_id = nvmStripeTableLookup(nvm_buf_tag.stripe_id, hashcode);
        if(lru_buf_id >= 0)
        {
            nvm_buf_hdr_lru = &nvm_buf_descriptors_lru_stripe[lru_buf_id];
        }
        else {
            nvm_buf_hdr_lru = getLRUStripe();
        }
...
{
    long i;
    NVMBufferDescStripe *nvm_buf_hdr_lru;
    if(nvm_buf_control_lru_stripe->first_freenvm >= 0)
    {
        nvm_buf_hdr_lru = nvm_buf_descriptors_lru_stripe[nvm_buffer_control_lru->first_lru];
